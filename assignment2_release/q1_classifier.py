...
class SoftmaxModel(Model):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    self.input_placeholder = tf.placeholder(tf.float32, [self.config.batch_size, self.config.n_features], name='INPUT')
    self.labels_placeholder = tf.placeholder(tf.float32, [self.config.batch_size, self.config.n_classes], name='LABEL')
    ### END YOUR CODE
  ...
  def add_model(self, input_data):
    ...
    with tf.variable_scope('model'):
      W = tf.get_variable('W', shape=[self.config.n_features,self.config.n_classes], initializer=tf.random_normal_initializer(mean=0,stddev=1))
      b = tf.get_variable('b', shape=[1,self.config.n_classes], initializer=tf.constant_initializer(0.0))
