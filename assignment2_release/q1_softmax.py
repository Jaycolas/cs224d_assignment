...
def softmax(x):
  ...
  x_minus_max = tf.subtract(x, max)
  e_x = tf.exp(x_minus_max)
  e_x_sum = tf.reduce_sum(e_x, axis=1, keep_dims=True)
  out = tf.div(e_x, e_x_sum)
  ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
  ...
  """
  ### YOUR CODE HERE
  y_yhat = tf.multiply(tf.to_float(y), tf.log(yhat))
  out = -tf.reduce_sum(y_yhat)
  ### END YOUR CODE
  return out
