...
class NERModel(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    self.labels_placeholder  = tf.placeholder(tf.float32, [None, self.config.label_size], name='labels_placeholder')
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      with tf.variable_scope('add_embedding'):
        xavier_initializer=xavier_weight_init()
        embeddings_shape = [len(self.wv), self.config.embed_size]
        embeddings = tf.get_variable('embeddings', shape= embeddings_shape, initializer=xavier_weight_init())
        window = tf.nn.embedding_lookup(params=embeddings, ids=self.input_placeholder)
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope('Layer'):
      W_shape = [self.config.window_size*self.config.embed_size, self.config.hidden_size]
      b1_shape = [self.config.hidden_size,]
      U_shape = [self.config.hidden_size,self.config.label_size]
      b2_shape =[self.config.label_size,]
      xavier_initializer = xavier_weight_init()
      W = tf.get_variable('W', shape=W_shape, initializer=xavier_weight_init())
      b1 = tf.get_variable('b1', shape=b1_shape, initializer=xavier_weight_init())
      U = tf.get_variable('U', shape=U_shape, initializer=xavier_weight_init())
      b2 = tf.get_variable('b2', shape=b2_shape, initializer=xavier_weight_init())

      h = tf.matmul(window,W)+b1
      h_fc1 = tf.nn.relu(h)
      h_fc1 = tf.nn.dropout(h_fc1, keep_prob=self.dropout_placeholder)
      h_fc2 = tf.matmul(h_fc1,U)+b2
      h_fc2 = tf.nn.dropout(h_fc2, keep_prob=self.dropout_placeholder)
  ...
  def add_loss_op(self, y):
    ...
    loss+=self.config.l2*tf.get_collection(key='l2_loss')[0]
    ### END YOUR CODE
    return loss
